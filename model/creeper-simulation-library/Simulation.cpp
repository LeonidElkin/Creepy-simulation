...
Simulation::Simulation(const Point size, double creeperExplodeRadius_, size_t creepersNum, double moveRadius,
                       FuncType funcType)
  ...
  auto creeperParams = std::make_shared<CreepersParams>(moveRadius, creeperExplodeRadius_ * creeperExplodeRadius_,
                                                        leftDownBound_, rightUpBound_);

  creepers_ = std::views::iota(0LU, creepersNum) |
              std::views::transform([&](auto i) { return Creeper(i, creeperParams); }) | std::ranges::to<std::vector>();
}
