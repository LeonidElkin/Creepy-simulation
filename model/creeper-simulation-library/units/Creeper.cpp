...
void Creeper::updateState(const Unit &another, const std::function<double(Point, Point)> &distanceFun) {
  ...
  const auto distance = distanceFun(getCoord(), another.getCoord());
...
CreepersParams::CreepersParams(const double moveRadius, const double explodeRadius,
                               const std::shared_ptr<FieldParams> &fieldParams)
    : UnitsParams(moveRadius, fieldParams), explodeRadius(explodeRadius) {}
